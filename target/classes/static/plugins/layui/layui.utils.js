...
layer.ready(function() {
	...
	table.on('tool(layui-table-filter)', function(obj) {
		...
		//详情
		if (event === 'detailRed') {
			$.dialog.openContent('红包详情','ignRedDetail?id='+data.id,width,height);
			return;
		}
		//详情
		if (event === 'detailPingJia') {
			$.dialog.openContent('问卷详情','detail?id='+data.id+'&serviceType='+data.serviceType,1200,800);
			return;
		}
		//编辑
		if (event === 'edit') {
		...
		//编辑员工
		if (event === 'editUser') {
			$.dialog.openSimple('编辑','edit?id='+data.id+'&employeeType='+data.employeeType,width,height);
			return;
		}
		//删除
		if (event === 'del') {
